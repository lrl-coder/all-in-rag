<template>
  <main class="shell">
    <aside class="sidebar">
      ...
      <nav class="nav">
        <button v-for="item in navItems" :key="item.id" :class="{ active: activeView === item.id }" @click="activeView = item.id">
    ...
    <section class="workspace">
      <header class="topbar">
        <div>
          <p class="eyebrow">智能烹饪问答工作台</p>
          <h2>{{ currentTitle }}</h2>
        </div>
        <div class="actions">
          <button class="icon-btn" title="刷新状态" @click="loadEverything"><RefreshCw /></button>
          <button class="primary" :disabled="busy" @click="initializeSystem"><Power /> 初始化</button>
          <button class="primary muted" :disabled="busy" @click="buildKnowledge"><Database /> 构建</button>
        </div>
      </header>
      ...
      <section v-if="activeView === 'chat'" class="chat-layout">
        <div class="chat-panel">
            ...
            <article v-for="message in messages" :key="message.id" :class="['message', message.role]">
              ...
              <div class="bubble">
                ...
                <div v-if="message.analysis" class="analysis-strip">
                  ...
                  <span>关系密度 {{ percent(message.analysis.relationship_intensity) }}</span>
          ...
          <form class="composer" @submit.prevent="askQuestion">
            <textarea v-model="question" placeholder="例如：家里有鸡胸肉和西兰花，推荐一道低脂晚餐，并说明为什么这样搭配。" />
            <div class="composer-actions">
              ...
              <button type="submit" class="send" :disabled="busy || !question.trim()"><Send /> 发送</button>
            </div>
          </form>
        </div>
        ...
        <aside class="inspector">
          ...
          <MetricCard label="传统检索" :value="routeStats.traditional_count || 0" />
          <MetricCard label="Graph RAG" :value="routeStats.graph_rag_count || 0" />
          <MetricCard label="组合策略" :value="routeStats.combined_count || 0" />

          <div class="quick-card">
            <h3>快速试问</h3>
            <button v-for="sample in samples" :key="sample" @click="question = sample">{{ sample }}</button>
          </div>
        </aside>
      </section>
      ...
      <section v-if="activeView === 'stats'" class="dashboard-grid">
        ...
        <MetricCard label="食材数量" :value="knowledge.total_ingredients || 0" />
        <MetricCard label="烹饪步骤" :value="knowledge.total_cooking_steps || 0" />
        <MetricCard label="文本块" :value="knowledge.total_chunks || 0" />

        <div class="wide-panel">
          <h3>知识库分布</h3>
          <div class="bars">
            <DataBar v-for="item in categoryItems" :key="item.name" :label="item.name" :value="item.value" :max="categoryMax" />
          </div>
        </div>
        ...
        <div class="wide-panel">
          <h3>检索路由</h3>
          <div class="route-mix">
            <DataBar label="传统混合检索" :value="routeStats.traditional_count || 0" :max="routeMax" />
            <DataBar label="图结构检索" :value="routeStats.graph_rag_count || 0" :max="routeMax" />
            <DataBar label="组合策略" :value="routeStats.combined_count || 0" :max="routeMax" />
          </div>
        </div>
      </section>
      ...
      <section v-if="activeView === 'routing'" class="two-column">
        <div class="wide-panel">
          <h3>路由分析</h3>
          <textarea v-model="routingQuestion" class="analysis-input" placeholder="输入问题，查看系统会选择哪种检索策略。" />
          <button class="primary" :disabled="busy || !routingQuestion.trim()" @click="explainRoute"><Route /> 分析问题</button>
        </div>
        ...
        <div class="wide-panel">
          ...
          <div class="form-grid">
            <label v-for="field in configFields" :key="field.key">
              ...
              <input v-model="configDraft[field.key]" :type="field.type || 'text'" />
            </label>
          </div>
          <div class="panel-actions">
            <button class="primary" :disabled="status.initialized" @click="saveConfig"><SlidersHorizontal /> 保存配置</button>
            <span v-if="status.initialized" class="muted-text">系统启动后配置会锁定。</span>
          </div>
        </div>

        <div class="danger-panel">
          <h3>知识库维护</h3>
          <p>重建会删除当前 Milvus 集合并重新加载 Neo4j 图数据、分块、建索引。</p>
          <button class="danger" :disabled="busy" @click="rebuildKnowledge"><Trash2 /> 重建知识库</button>
        </div>
      </section>
    </section>
  </main>
...
const messages = ref([
  {
    id: newId(),
    role: "assistant",
    content: "控制台已连接。先初始化系统并构建知识库，然后就可以开始询问菜谱、食材替换、步骤推理和搭配建议。",
  },
]);
...
const currentTitle = computed(() => navItems.find((item) => item.id === activeView.value)?.label || "控制台");
const categoryItems = computed(() => Object.entries(knowledge.value.categories || {}).slice(0, 8).map(([name, value]) => ({ name, value })));
const categoryMax = computed(() => Math.max(1, ...categoryItems.value.map((item) => item.value)));
const routeMax = computed(() => Math.max(1, routeStats.value.traditional_count || 0, routeStats.value.graph_rag_count || 0, routeStats.value.combined_count || 0));
...
async function runAction(fn) {
  ...
  try {
    ...
    messages.value.push({ id: newId(), role: "assistant", content: `操作失败：${error.message}` });
  } finally {
    busy.value = false;
  }
}
...
function rebuildKnowledge() {
  ...
  runAction(() => api("/api/knowledge/rebuild", { method: "POST", body: JSON.stringify({ confirm: "REBUILD" }) }));
}

function saveConfig() {
  runAction(() => api("/api/config", { method: "PUT", body: JSON.stringify(configDraft.value) }));
}
...
async function explainRoute() {
  ...
  try {
    ...
    routeAnalysis.value = { recommended_strategy: "error", reasoning: error.message, query_complexity: 0, relationship_intensity: 0, confidence: 0 };
  } finally {
    busy.value = false;
  }
}
